...
class InceptionV3(nn.Module):
    ...
    def __init__(self,
        ...
        if use_fid_inception:
            inception = fid_inception_v3(self.weights_path)
        else:
...
def fid_inception_v3(weights_path):
...
def calculate_fid_given_paths(paths, batch_size, device, dims, num_workers=1, weight_path='./'):
    ...
    model = InceptionV3(weight_path, [block_idx]).to(device)
...
def downloadFile(obs_path, local_path, result):
    obs = OBSHandler()
...
def load_paths(local_path, team_file_name, team_obs_path, result):
    ...
    if result['status'] != -1:
        ...
        real_path = un_zip_real(REAL_FILE_NAME, real_local_path)
        return real_path, fake_path
    return '', ''


def calcu_fid(local_path, team_obs_path):
    result = {
        "status": 200,
        "msg": "评分成功",
        "data": -1
